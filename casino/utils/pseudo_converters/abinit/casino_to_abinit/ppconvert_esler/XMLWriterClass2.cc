...
void XMLAttribute::Write(string &out) {
 stringstream str;
 str << Name << "=\"" << Content << "\"";
 out = str.str();
}
...
void XMLElement::Indent(ostream &out) {
 for (int i=0; i<Level; i++) out << "  ";
}


void XMLElement::Write(ostream &out) {
 Indent(out);
 out << "<" << Name;
 ...
 if (Content == "" && Children.size()==0)
  ...
  out << ">\n";
  out << Content;
  for (int i=0; i<Children.size(); i++) Children[i]->Write(out);
  Indent(out);
  out << "</" << Name << ">\n";
 }
}
...
void XMLElement::AddContent(vector<double> &data) {
 stringstream str;
 ...
 if ((data.size()%3) != 0) str << "\n";
 Content += str.str();
}


bool XMLWriterClass::StartDocument(string fname, string version,
...
void XMLWriterClass::StartElement(string name) {
 int level = Elements.size();
 XMLElement* elem = new XMLElement(name, level);
 if (level > 0) Elements.back()->AddElement (elem);
 Elements.push_back(elem);
}


void XMLWriterClass::EndElement() {
 if (Elements.size() > 1) Elements.pop_back();
}


void XMLWriterClass::FullEndElement() { Elements.pop_back(); }


void XMLWriterClass::WriteAttribute (string name, string content) {
 XMLAttribute *attr = new XMLAttribute (name, content);
 Elements.back()->AddAttribute (attr);
}


void XMLWriterClass::WriteAttribute (string name, double val,
 bool scientific) {
...
void XMLWriterClass::WriteAttribute (string name, int val) {
 stringstream content;
 content << val;
 XMLAttribute *attr = new XMLAttribute (name, content.str());
 Elements.back()->AddAttribute (attr);
...
void XMLWriterClass::WriteData(vector<double> data) {
 Elements.back()->AddContent (data);
}


void XMLWriterClass::WriteElement (string name, vector<double> data) {
 int level = Elements.size();
 XMLElement *elem = new XMLElement (name, level);
