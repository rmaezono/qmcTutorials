...
CONTAINS
  ...
  SUBROUTINE read_casino(iunps,nofiles, waveunit)
    !     ----------------------------------------------------------
    !
    !     Reads in a CASINO tabulated pp file and its associated
...
!      WRITE(6,"(I3,F5.2)") atomic_number( upf_in%psd ),upf_in%zp ! MDT 12.2014
       WRITE(6,"(I3,F8.2)") atomic_number( upf_in%psd ),upf_in%zp
       WRITE(6,*) "Energy units (rydberg/hartree/ev):"
       WRITE(6,*) "rydberg"
       WRITE(6,*) "Angular momentum of local component (0=s,1=p,2=d..) for &
        &DFT and QMC"
       WRITE(6,"(I2,1X,I2)") upf_in%lloc,upf_in%lloc
