...
!Local variables -------------------------------------
 integer :: ii,jj,io,iocc,nelecs,ikpt,iband,isppol,icg,iband_kpt_shift,&
            &iband_sppol_shift,icg_shift,ikg,ikgfull,ikg_shift,icgfull,&
            &icgfull_shift,ikpt_shift,nband1,nband2,nkgfull
 integer:: kgfull(3,mpw*mkmem)
 integer:: kgmap(mpw*mkmem)
 real(dp):: kptscart_qmc(3,nkpt)
 real(dp), allocatable :: cgfull(:,:)
 integer :: ierr=0
 integer,parameter :: r2s_length=80
 real(dp),allocatable :: xcart_qmc(:,:)
 real(dp):: gcart_qmc(3)
 real(dp):: norm
...
! Go away if I'm not the master node.
 ...
 am_master=.true.
           ...
           if(my_node/=0)am_master=.false.
           if(mpi_enreg%paral_level>1)then
            write(message,'(a,a,a,i0,a)')' outqmc : ERROR -',ch10,&
             &'  paral_level=',mpi_enreg%paral_level,' : Parallelization over &
             &k-points is not currently supported'
...
! Open CASINO pwfn.data file
 ...
 if (dtset%kptopt==1) then
  ...
  write(message,'(a)')' outqmc : ERROR - kptopt=1 so k-points have been &
 ...
 do ikpt=1,nkpt
  if (dtset%istwfk(ikpt)/=1) then
   close(io,status='delete')
   write(message,'(a,a,a,i0,a,i0,a,a,a,a,a)')' outqmc : ERROR -',ch10,&
...
! Write the required quantities to pwfn.data
 write(io,"('QMC trial wave function file for CASINO generated by ABINIT &
            &(www.abinit.org).')")
 ...
 select case (dtset%ixc)
  ...
  case(10)
   dft_functional='Diff. between ixc=7 and 9; use with accurate RPA corr. &
    &energy'
  case(11)
  ...
  case(13)
   dft_functional='GGA (potential: van Leeuwen-Baerends ; energy: Perdew-Wang &
    &92)'
  case(14)
  ...
  case(22)
   dft_functional='Fermi-Amaldi xc for TDDFT with Burke-Petersilka-Gross &
    &hybrid xc kernel'
  case default
   dft_functional='Unknown type.'
 end select
 write(io,"(' ABINIT ixc = ',i0,' : ',a)")dtset%ixc,trim(dft_functional)
 ...
 select case(dtset%nspden)
  ...
  case(4)
   close(io,status='delete')
   write(message,'(a)')' outqmc : ERROR - nspden=4 but CASINO cannot yet &
    &deal with non-collinear spins.'
   call wrtout(6,message,'PERS')
   call leave_new('PERS')
  case default
   close(io,status='delete')
 ...
 tmpr=r2s(results_gs%ek,'(f24.14)')
 write(io,'(1x,a)')trim(tmpr)
 write(io,"('Local potential energy (Local pseudopotential energy eei +&
  &pseudopotential core-core energy eii)')")
 tmpr=r2s((results_gs%eei+results_gs%eii),'(f24.14)')
 write(io,'(1x,a)')trim(tmpr)
 write(io,"('Non-local potential energy')")
 tmpr=r2s(results_gs%enl,'(f24.14)')
 write(io,'(1x,a)')trim(tmpr)
 write(io,"('Electron-electron energy (Hartree Energy + Exchange-Correlation &
  &Energy)')")
 tmpr=r2s((results_gs%ehart+results_gs%enxc),'(f24.14)')
 write(io,'(1x,a)')trim(tmpr)
 write(io,"('Ion-ion energy')")
 tmpr=r2s(results_gs%eew,'(f24.14)')
 write(io,'(1x,a)')trim(tmpr)
 ...
 write(io,'(x,i0)')nelecs
 write(io,*)
 write(io,"('GEOMETRY'/'--------')")
 write(io,"('Number of atoms per primitive cell')")
 write(io,'(x,i0)')dtset%natom
 ...
 do ii=1,dtset%natom
  ...
  write(io,'(x,i0,3(x,a))')jj,trim(tmpr),trim(tmpr2),trim(tmpr3)
 enddo
...
! These are checked by CASINO and it will complain if they are not as expected.
  ...
  do isppol=1,nsppol
   do iband=1,dtset%nband(ikpt+(isppol-1)*nkpt)
    if(occ(iocc)/=int(occ(iocc)))then
     write(message,'(a,a,a,i0,a,i0,a,i0,a,f10.8,a)')' outqmc : WARNING -',&
 ...
 icgfull_shift=0
 do ikpt=1,nkpt
  write(io,"('k-point # ; # of bands (up spin/down spin) ; k-point coords &
   &(au)')")
  if(nsppol==2)then
  ...
  write(io,'(3(x,i0),3(x,a))')ikpt,nband1,nband2,trim(tmpr),trim(tmpr2), &
   &trim(tmpr3)
  do isppol=1,nsppol
   if (isppol==2) then
   ...
   do iband=1,dtset%nband(ikpt)
    ...
    write(io,'(2(x,i0),x,a)')iband,isppol,tmpr
    write(io,"('Eigenvector coefficients')")
    do icgfull=1,nkgfull
     write(io,"(x,'(',e23.16,',',e23.16,')')")cgfull(1:2,icgfull+icgfull_shift)
    enddo !icgfull
    icgfull_shift=icgfull_shift+nkgfull
   enddo !iband
  enddo !isppol
